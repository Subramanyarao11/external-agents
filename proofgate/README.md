...
`ProofGate / evaluate` PR check, in the GitHub job summary, as one bot-authored
sticky PR evidence report, and as a retained Change Passport artifact. The
sticky report is updated instead of posting a new comment on every push. It
summarizes the verdict, Entire provenance, Graph impact, test coverage,
historical context, policy reasons, and review focus from allowlisted metadata.
Manual `workflow_dispatch` runs remain available for the Control Room's
exact-SHA approval roundtrip.
