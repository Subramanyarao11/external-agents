...
// CaptureStart captures a run using automatic provider detection. Callers that
// need to pin a provider or model use CaptureStartWithProvider.
func (a *Agent) CaptureStart(sessionID, prompt string) (CaptureResult, error) {
	return a.CaptureStartWithProvider(sessionID, prompt, "auto", "")
}

func (a *Agent) CaptureStartWithProvider(sessionID, prompt, provider, model string) (CaptureResult, error) {
...
// CaptureFinish finalizes a run using automatic provider detection. Callers
// that need to pin a provider, prompt or model use CaptureFinishWithProvider.
func (a *Agent) CaptureFinish(executionFile, sessionID string) (CaptureResult, error) {
	return a.CaptureFinishWithProvider(executionFile, sessionID, "auto", "", "")
}

func (a *Agent) CaptureFinishWithProvider(executionFile, sessionID, provider, prompt, model string) (CaptureResult, error) {
	...
	// The stored session must stay byte-identical to the supplied transcript,
	// so provider normalization and context enrichment remain in memory and
	// only feed capture metadata.
	enriched := addCaptureContext(messages, prompt, model)
	...
	if resolvedModel == "" {
		resolvedModel = messagesModel(enriched)
	}
	if resolvedModel != "" {
...
func messagesModel(messages []sdkMessage) string {
